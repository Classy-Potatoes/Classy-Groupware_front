...
function ProjectScheduleList({postSuccess, projectCode, schedule, memberId}) {
    ...
    useEffect(() => {
        if (modifyMode && schedule) {

            const initialAttendants = schedule.managers.map((manager) => ({
                value: manager.infoCode,
                label: manager.memberName
            }));

            setAttendants(initialAttendants);
        }
    }, [modifyMode, schedule]);
    ...
    const clickedRegist = () => {
            ...
            const formattedStartDate = startDate ? formatDate(addOneDay(startDate)) : schedule.scheduleStartDate.split('T')[0];
            const formattedEndDate = endDate ? formatDate(addOneDay(endDate)) : schedule.scheduleEndDate.split('T')[0];
            const formattedStartTime = startTime ? formatTime(startTime) : schedule.scheduleStartDate.split('T')[1].slice(0, 5);
            const formattedEndTime = endTime ? formatTime(endTime) : schedule.scheduleEndDate.split('T')[1].slice(0, 5);
