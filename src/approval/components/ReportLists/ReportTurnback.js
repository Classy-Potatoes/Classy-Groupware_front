...
import {callApprovalStatusUpdateAPI, callReportTurnbackAPI, callReportWaitingAPI} from "../../apis/ReportAPICalls";
...
function ReportTurnback() {

    const dispatch = useDispatch();
    const [currentPage, setCurrentPage] = useState(1);
    const [selectAll, setSelectAll] = useState(false);
    const {reportTurnback} = useSelector(state => state.approvalReducer);
    ...
    useEffect(() => {
        dispatch(callReportTurnbackAPI({currentPage}));
    }, [currentPage]);
    ...
    const onClickSearchHandler = e => {
        ...
        navigate(`/approval/report/search-turnback?documentTitle=${documentTitleSearch}&startDate=${formattedStartDate}&endDate=${formattedEndDate}&page=${currentPage}`);
    }
                ...
                {reportTurnback &&
                    <div className="reportTable">
                        ...
                        <table className="Report-table">
                            ...
                            {reportTurnback.data.map(report => (
                                <tr
                        ...
                        <ApprovalPagingBar pageInfo={reportTurnback.pageInfo} setCurrentPage={setCurrentPage}/>
                    </div>
